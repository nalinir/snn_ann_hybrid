...
"""all classes from Rossbroich et al. (2022), https://github.com/fmi-basel/stork/tree/main/stork"""
...
class ActivityRegularizer:
    ...
    def __call__(self, output, gamma=None):  # group):
        """Expects input with (batch x time x units)"""
        # act = group.get_out_sequence()  # get output
        # cnt = torch.sum(act, dim=1)  # get spikecount
        print(output.shape)
        cnt = torch.sum(output, dim=1)  # get spikecount over time

        # if population-level regularizer, calculate mean across defined dims
        if self.dims:
            # --- Population-level regularizer (e.g., Upper Bounds) ---
            if gamma is not None:
                # Apply gamma normalization for the population average
                denominator = torch.sum(gamma) + 1e-6
                cnt = torch.sum(cnt, dim=self.dims) / denominator
            else:
                # Original logic: standard mean
                cnt = torch.mean(cnt, dim=self.dims)
        elif gamma is not None:
            cnt = cnt / (gamma + 1e-6) # Rescale it back to what it would be without gamma
