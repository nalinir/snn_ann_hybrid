...
def data_split_nmnist(config):
    ...
    train_subset_indices = get_representative_subset_indices(train_dataset, train_limit*2)
    ...
    train_x_data = torch.stack([
        preprocess_spike_events(train_subset[i][0],  # First element is the events
        nb_steps=config['nb_steps'],
        nb_units=config['nb_inputs'],
        time_step=config['time_step']
    ) for i in train_indices])
    
    train_y_tensor = torch.tensor([train_subset[i][1] for i in train_indices], dtype=torch.int64)
    train_tensor_dataset = TensorDataset(train_x_data, train_y_tensor)

    # Process validation data
    val_x_data = torch.stack([
        preprocess_spike_events(train_subset[i][0],
        nb_steps=config['nb_steps'],
        nb_units=config['nb_inputs'],
        time_step=config['time_step']
    ) for i in val_indices])
    val_y_tensor = torch.tensor([train_subset[i][1] for i in val_indices], dtype=torch.int64)
    val_tensor_dataset = TensorDataset(val_x_data, val_y_tensor)

    # Process test data
    test_x_data = torch.stack([
        preprocess_spike_events(test_subset[i][0],
        nb_steps=config['nb_steps'],
        nb_units=config['nb_inputs'],
        time_step=config['time_step']
    ) for i in range(len(test_subset))])
    test_y_tensor = torch.tensor([test_subset[i][1] for i in range(len(test_subset))], dtype=torch.int64)
