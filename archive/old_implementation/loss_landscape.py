...
def visualize_loss_landscape_3d(
    ...
    for i, a in enumerate(tqdm(alphas, desc="α")):
        for j, b in enumerate(betas):
            ...
            # output = model(xb)
            # predictions, auxiliary_outputs = model(xb)

            predictions, auxiliary_outputs, _, _ = model(xb)
            max_over_time, _ = torch.max(predictions, dim=1)  # [batch, num_classes]
            # print("output shape:", max_over_time.shape)
            # print("yb shape:", yb.shape)
            # print("yb dtype:", yb.dtype)
            # print("unique yb:", yb.unique())
            loss_grid[j, i] = criterion(max_over_time, yb).item()
